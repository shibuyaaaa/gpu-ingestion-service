...
class Settings:
    ...
    download_workers: int = _int_env("DOWNLOAD_WORKERS", _int_env("PREP_WORKERS", 2))
    process_workers: int = _int_env("PROCESS_WORKERS", _int_env("POSTPROCESS_WORKERS", 6))
    ...
    youtube_auth_recovery_enabled: bool = _bool_env("YOUTUBE_AUTH_RECOVERY_ENABLED", True)
    youtube_auth_recovery_interval_seconds: float = _float_env("YOUTUBE_AUTH_RECOVERY_INTERVAL_SECONDS", 300.0)
    youtube_auth_recovery_batch_size: int = _int_env("YOUTUBE_AUTH_RECOVERY_BATCH_SIZE", 100)
    ...
    htdemucs_model: str = os.getenv("HTDEMUCS_MODEL", "htdemucs_ft")
    ...
    gpu_job_sample_interval_seconds: float = _float_env("GPU_JOB_SAMPLE_INTERVAL_SECONDS", 0.5)
